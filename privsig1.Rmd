...
raw_html_return <- read_html("https://raw.githubusercontent.com/alc28/privsig/main/test_files/PERSONAL_DATA_DISCLOSURE_multiple_pd_line.md") %>%
  html_text()


text1 <- raw_html_return 
  

str_locate_all(text1, "\\[[x|X]\\]")

text2 <- str_extract_all(text1, "\\[[x|X]\\].*\n") %>% unlist() %>% str_remove_all(., "\n")
